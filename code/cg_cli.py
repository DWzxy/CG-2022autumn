...
#                       canvas[height - 1 - y, x] = color
                ...
                algorithm = line[6]
                item_dict[item_id] = [
                    'line', [[x0, y0], [x1, y1]], algorithm,
                    np.array(pen_color)
                ]

            elif line[0] == 'drawPolygon':
                item_id = line[1]
                item_dict[item_id] = ['polygon', [], '', np.array(pen_color)]
                for i in range(2, len(line) - 1, 2):
                    item_dict[item_id][1].append(
                        [int(line[i]), int(line[i + 1])])
                item_dict[item_id][2] = line[-1]
                ...
                x = int(line[2])
                y = int(line[3])
                p_list = item_dict[item_id][1]  #图形的坐标列表
                item_dict[item_id][1] = alg.translate(p_list, x, y)
                ...
                item_dict[item_id][1] = alg.scale(p_list, x, y, r)
                if (len(item_dict[item_id][1]) == 0):
                    del item_dict[item_id]  #如果裁剪没了就不要了
